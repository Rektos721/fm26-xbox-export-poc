...
DWORD WINAPI BootstrapThread(LPVOID param) {
    ...
    const std::wstring payloadPath = dir + L"\\payload.dll";
    if (GetFileAttributesW(payloadPath.c_str()) == INVALID_FILE_ATTRIBUTES) {
        LogLine(dir, L"payload.dll not found; bootstrap idle");
        return 0;
    }

    HMODULE payload = LoadLibraryW(payloadPath.c_str());
    if (!payload) {
        LogLine(dir, L"LoadLibraryW(payload.dll) failed: " + std::to_wstring(GetLastError()));
        return 1;
    }
